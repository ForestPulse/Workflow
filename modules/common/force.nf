...
// higher-level processing
// it is expected to be stored as a 1-tile cube on a folder named as the 'product'
// .prm file should reflect that on higher level and provenance
// other created files should reflect it as well
// removing the last item of the tuple for even easier chaining?
process force_higher_level_chain {
  ...
  output:
  tuple path("${product}/"), path(maskdir), val(tile_ID), val(tile_X), val(tile_Y), val(product) //, optional: true

  """
  mkdir "${product}"
  sed -i "/^DIR_HIGHER /c\\DIR_HIGHER = ${product}" ${parfile}
  sed -i "/^DIR_PROVENANCE /c\\DIR_PROVENANCE = ${product}" ${parfile}
  force-higher-level "${parfile}"
  """
